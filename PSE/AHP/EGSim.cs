...
namespace PSE.AHP
{
    ...
    class EGSim
    {
        ...
        ////中间变量机器状态变量
        public int[] ms;////ms[i]=0,1,2,3,4,5  0--空闲，1--加工,2--堵塞,3---故障
       ...
       //记录特殊算法
        ...
        //////辅助函数
        public int tryAddStart(int mid)
        {
            if (mid < 0 || mid >= MachineNum) return 0;
            if (ms[mid] > 0) return 0;
            if(mid==0)
            {
                evList.AddEvent(currentTime, mid , 1, "start");
                return 1;
            }else
            {
                if(wip[mid-1]>0)
        ...
        public int unBlock(int mid)
        {
            if (mid < 0 || mid >= MachineNum) return 0;
            if(ms[mid]==2)
            {
                evList.AddEvent(currentTime, mid, 1, "end");
                return 1;
            }
